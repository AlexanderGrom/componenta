...
type parser struct {
	...
	storage  Env    // Собранные переменные
	key      string // Найденный ключ
	value    string // Найденные значения
}
...
func (self *parser) Parse(r []rune) Env {
	self.textBuf = r
	self.textLen = len(r)
...
func (self *parser) extractEnv() Env {
	for !self.isEOF() {
		self.state()
	}
	return self.storage
...
func (self *parser) matchValue() {
	...
	if self.isQuote(self.curChar) {
		...
		for !self.isEOF() && (self.curChar != quote || escape) {
			if self.isDollar(self.curChar) && !escape {
				...
				if val == "" {
					...
					self.removePos()
					buff.WriteString(self.getEnv(val))
				}
		...
		self.value = buff.String()
		self.moveNextPos()
		self.skipSpaces()
	} else {
		escape := false
		buff := bytes.Buffer{}
		for !self.isNL(self.curChar) && !self.isEOF() && !self.isComment(self.curChar) {
			if self.isDollar(self.curChar) && !escape {
				self.savePos()
				val := self.grabVar()
				if val == "" {
		...
		self.value = strings.TrimSpace(buff.String())
	}

	self.saveState(self.saveEnv)
	self.skipLine()
}

// Сохранение найденной переменной и начало поиска нового ключа
func (self *parser) saveEnv() {
...
func (self *parser) getEnv(name string) string {
	if val, ok := self.storage[name]; ok {
		return val
	}
